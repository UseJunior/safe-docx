...
def terminalResponse (code detail : String) : Json :=
  Json.mkObj
    ...
    , ("noteIntegrityIssues", Json.arr #[terminalIssue code detail])
    ]
...
def main : IO Unit := do
  ...
  let response :=
    if mode == "strings" then
      terminalResponse "NOTE_EVIDENCE_STRING_BUDGET_EXCEEDED"
        "protocol v5 escaped evidence string budget exceeded"
    else
      terminalResponse "NOTE_ISSUE_LIMIT_EXCEEDED"
        "protocol v5 aggregate ordinary issue limit exceeded"
  if response.compress.toUTF8.size > 2621440 ||
      (response.compress ++ "\n").toUTF8.size > 2621441 then
    throw (IO.userError "canonical terminal exceeds protocol-v5 legal envelope")
