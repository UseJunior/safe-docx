...
namespace ProtocolV7ProjectionDriftWitnesses
...
def fixtureRecord (side : Tier2.RelationshipStorySelector.VerifierSide)
  ...
  let noteEvidence := fixtureSide side parseEvidence issues
  let sources := noteEvidence.sources
  let markerScans := retainedCommentSourceScansOfEvidence [] noteEvidence
  let markerSourceSet := retainedCommentMarkerSourceSetV7 side markerScans
  let markerScanResult :=
    retainedCommentMarkerScanForRelationshipV7 false markerSourceSet markerScans
  {
  packagePath := "/request-bound/fixture.docx"
  packageBytes := fixturePackageBytes
  packageReadCount := 1
...
def evaluateProductionCase (request : RunRequestCoreRequest) :
  ...
  let result ← finishRunRequestCore request semanticRequest semanticResponse
    semanticStdout
  if productionTypedCommentChecksV7 request result then
    return result
  else
    throw "production response diverges from typed protocol-v7 projection"
...
def baselineAgrees (requestResult : Except String RunRequestCoreRequest) : Bool :=
  ...
  | .ok result =>
    ...
    fieldValue result.response "protocolVersion" == toJson (7 : Nat) &&
    fieldValue result.response "checker" ==
      toJson "safe-docx-lean-conventional-main-comment-range-integrity-checker" &&
...
def run : IO Unit := do
  for (name, passed) in witnessResults do
    unless passed do
      throw (IO.userError s!"realizable protocol-v7 production drift witness failed: {name}")
  IO.println s!"realizable protocol-v7 production drift witnesses passed: {witnessResults.length - 1}"

end ProtocolV7ProjectionDriftWitnesses

#eval ProtocolV7ProjectionDriftWitnesses.run
