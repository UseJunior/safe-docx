...
namespace ProtocolV6OrdinaryEnvelopeWitness

open Lean

def ordinaryEscapedStringBytes : Nat := 1571840
def ordinaryLegalUpperEnvelope : Nat := 2624704
...
def commentStorySide (status : String) : Json :=
  if status == "absent" then
    ...
    Json.mkObj
      [ ("status", status)
      , ("relationship", commentIdentity)
      , ("partPresent", true)
      ]

def commentStory : Json :=
  Json.mkObj
    [ ("status", "failed")
    , ("original", commentStorySide "failed")
    , ("revised", commentStorySide "absent")
    , ("compared", commentStorySide "absent")
    , ("parsedTokenCounts", Json.mkObj
        [("original", 1), ("revised", 0), ("combined", 0)])
    ]

def commentInventory (side status : String) : Json :=
  Json.mkObj
    [ ("side", side)
    , ("status", status)
    , ("relationship", if status == "failed" then commentIdentity else Json.null)
    , ("referenceOccurrences", if status == "failed" then 1 else 0)
    , ("uniqueReferenceIds", if status == "failed" then 1 else 0)
    , ("definitions", 0)
    , ("unreferencedDefinitions", 0)
    , ("nonDirectDefinitions", 0)
    ]

def commentIssue : StateM Nat Json := do
  let detail ← padTo "comment reference has no direct definition" 256
  let canonicalId ← padTo "1" 64
  return Json.mkObj
...
def responseWithPadding : StateM Nat Json := do
  ...
  let issues ← (List.range 509).mapM noteIssue
  let commentsIssue ← commentIssue
  return Json.mkObj
    [ ("protocolVersion", 6)
    ...
    , ("commentInventories", Json.arr #[
        commentInventory "original" "failed",
        commentInventory "revised" "passed",
        commentInventory "compared" "passed"
      ])
    , ("commentIntegrityIssues", Json.arr #[commentsIssue])
    ]
...
def run : IO Unit := do
  ...
  if remaining != 0 then
    throw (IO.userError s!"near-envelope witness lacks {remaining} bytes of padding capacity")
  if charge != ordinaryEscapedStringBytes then
    throw (IO.userError s!"near-envelope witness charged {charge} escaped-string bytes")
  if bytes > ordinaryLegalUpperEnvelope then
    throw (IO.userError s!"near-envelope witness serialized to {bytes} bytes")
  IO.println response.compress

end ProtocolV6OrdinaryEnvelopeWitness

def main : IO Unit :=
  ProtocolV6OrdinaryEnvelopeWitness.run
