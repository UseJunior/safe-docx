...
const [manifest, registry, vocabularySource, evidenceResults] = await Promise.all([
...
test('rejects a newly introduced unclassified revision element', async () => {
  await assert.rejects(
    validateAdvancedRevisionClassification(cloneManifest(), [...vocabulary, 'futureChange'], registry, evidenceResults),
...
test('rejects an operation whose element-specific executable evidence is removed', async () => {
  ...
  await assert.rejects(
    validateAdvancedRevisionClassification(candidate, vocabulary, registry, evidenceResults),
...
test('rejects an executed evidence artifact with a missing mutation sentinel', async () => {
  ...
  await assert.rejects(
    validateAdvancedRevisionClassification(cloneManifest(), vocabulary, registry, candidateResults),
