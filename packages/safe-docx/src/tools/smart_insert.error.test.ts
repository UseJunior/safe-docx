...
describe('smart_insert validation + tag handling', () => {
  registerCleanup();
  ...
  it('rejects invalid position values and missing anchors', async () => {
    ...
    const invalidPosition = await smartInsert(opened.mgr, {
    ...
    const missingAnchor = await smartInsert(opened.mgr, {
      session_id: opened.sessionId,
      positional_anchor_node_id: 'jr_para_missing',
      new_string: 'new text',
  ...
  it('returns INSERT_ERROR for malformed inline tags', async () => {
    ...
    const malformed = await smartInsert(opened.mgr, {
      session_id: opened.sessionId,
      positional_anchor_node_id: paraId,
  ...
  it('strips hyperlink/definition tags in default mode and inserts clean text', async () => {
    ...
    const inserted = await smartInsert(opened.mgr, {
      session_id: opened.sessionId,
      positional_anchor_node_id: paraId,
    ...
    assertSuccess(inserted, 'smart_insert default tags');
  ...
  it('legacy definition mode converts definition tags to quoted terms', async () => {
    ...
    try {
      ...
      assertSuccess(inserted, 'smart_insert legacy definition');
