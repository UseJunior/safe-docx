...
import { DocxZip, parseXml, serializeXml } from '@usejunior/docx-primitives';
...
describe('assumption: strict failure transactionality', () => {
  registerCleanup();
  ...
  test('strict selector failure does not mutate session state or document XML', async () => {
    ...
    const opened = await openSession([], {
      xml,
      prefix: 'safe-docx-assumption-transactional-',
    });
