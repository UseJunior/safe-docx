...
export async function smartInsert(
  ...
  try {
    const resolved = await resolveSessionForTool(manager, params, { toolName: 'smart_insert' });
    ...
    // Strip <a> tags — hyperlinks are read-only and cannot be created via smart_insert.
    ...
    const res = session.doc.smartInsert({
