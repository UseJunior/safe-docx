...
describe('OpenSpec traceability: add-safe-docx-layout-format-controls', () => {
  ...
  test.openspec('build-time tooling may be external but optional')('Scenario: build-time tooling may be external but optional', async () => {
    const readmePath = fileURLToPath(new URL('../../README.md', import.meta.url));
    const readme = await fs.readFile(readmePath, 'utf-8');

    expect(readme).toContain('build-time');
    expect(readme.toLowerCase()).toContain('optional');
    expect(readme).toContain('format_layout');
  });
});
