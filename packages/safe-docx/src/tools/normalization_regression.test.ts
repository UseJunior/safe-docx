...
describe('normalization regression tests', () => {
  describe('Phase 3.1: jr_para_* IDs stable after normalization', () => {
    it('normalized and skip_normalization sessions produce same paragraph IDs for identical content', async () => {
      ...
      // NOT session-specific. Extract actual jr_para_* IDs and compare directly.
    ...
    const FIXTURES = [
      {
        ...
        path: path.resolve(testDir, '../../../docx-comparison/src/testing/fixtures/simple-word-change/original.docx'),
      },
      {
        label: 'split-run-boundary-change/revised.docx',
        path: path.resolve(testDir, '../../../docx-comparison/src/testing/fixtures/split-run-boundary-change/revised.docx'),
      },
    ];
