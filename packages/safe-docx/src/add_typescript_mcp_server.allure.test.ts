...
describe('OpenSpec traceability: add-typescript-mcp-server', () => {
  const test = testAllure.epic('OpenSpec Traceability').withLabels({ feature: 'add-typescript-mcp-server' });
  ...
  test.openspec('Read-only tools annotated correctly')('Scenario: Read-only tools annotated correctly', async () => {
    const readOnlyTools = new Set(['open_document', 'read_file', 'grep', 'get_session_status']);
  ...
  test.openspec('Destructive tools annotated correctly')('Scenario: Destructive tools annotated correctly', async () => {
    const destructiveTools = new Set(['smart_edit', 'smart_insert', 'format_layout', 'download']);
    for (const tool of MCP_TOOLS) {
  ...
  test.openspec('Concurrent sessions')('Scenario: Concurrent sessions', async () => {
    ...
    const editA = await smartEdit(mgr, {
  ...
  test.openspec('grep tool')('Scenario: grep tool', async () => {
    ...
    expect(String((res.matches as Array<{ para_id: string }>)[0]?.para_id)).toMatch(/^jr_para_[0-9a-f]{12}$/);
  ...
  test.openspec('smart_edit tool')('Scenario: smart_edit tool', async () => {
    const mgr = createTestSessionManager();
  ...
  test.openspec('smart_insert tool')('Scenario: smart_insert tool', async () => {
    const mgr = createTestSessionManager();
  ...
  test.openspec('Format-preserving text replacement')('Scenario: Format-preserving text replacement + bookmark-based targeting', async () => {
    ...
    const edit = await smartEdit(mgr, {
      session_id: sessionId,
