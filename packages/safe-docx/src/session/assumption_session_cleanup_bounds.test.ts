...
describe('assumption: session cleanup bounds (A19)', () => {
  registerCleanup();

  test('clear_session by session_id removes tmp artifacts for that session', async () => {
    const mgr = createTestSessionManager();
    const tmpDir = await createTrackedTempDir('safe-docx-assumption-cleanup-single-');
  ...
  test('clear_all with confirm removes tmp artifacts for all active sessions', async () => {
    const mgr = createTestSessionManager();
    const tmpDir = await createTrackedTempDir('safe-docx-assumption-cleanup-all-');
