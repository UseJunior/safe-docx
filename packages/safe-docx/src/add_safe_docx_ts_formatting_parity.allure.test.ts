...
describe('OpenSpec traceability: add-safe-docx-ts-formatting-parity', () => {
  const test = testAllure.epic('OpenSpec Traceability').withLabels({ feature: 'add-safe-docx-ts-formatting-parity' });
  ...
  test.openspec('smart_edit preserves mixed-run formatting')('Scenario: smart_edit preserves mixed-run formatting', async () => {
    const xml =
      `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>` +
      `<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">` +
      `<w:body>` +
  ...
  test.openspec('smart_insert preserves header/definition semantics')('Scenario: smart_insert preserves header/definition semantics', async () => {
    const xml =
      `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>` +
      `<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">` +
      `<w:body>` +
  ...
  test.openspec('header semantics accepted via tags for backward compatibility')('Scenario: header semantics accepted via tags for backward compatibility', async () => {
    ...
    const editB = await smartEdit(mgr, {
      session_id: sessionId,
