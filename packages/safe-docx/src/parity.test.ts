...
describe('Parity regression', () => {
  ...
  test('tool parity: open -> read -> grep -> edit -> insert -> download -> status', async () => {
    ...
    const firstDataLine = toonLines.find((l) => l.startsWith('jr_para_'));
    ...
    // Verify output document.xml has no jr_para_* bookmarks (cleaned).
    const outXml = await readDocumentXmlFromPath(outPath);
    expect(outXml.includes('jr_para_')).toBe(false);
  ...
  test('grep reports para_index_1based for matches in later paragraphs', async () => {
    ...
    const ids = String(read.content)
      .split('\n')
      .filter((line) => line.startsWith('jr_para_'))
  ...
  test('open/read preserves existing jr_para_* when stacked with edit-* bookmark on same paragraph', async () => {
    ...
    const xml =
      ...
      `<w:bookmarkStart w:id="41" w:name="jr_para_keepme"/>` +
      `<w:p>` +
    ...
    const read2 = await readFile(mgr, { session_id: sessionId, node_ids: ['jr_para_keepme'], format: 'simple' });
    assertSuccess(read2, 'read2');
  ...
  test('duplicate signature-block lines remain uniquely addressable with persisted jr_para ids', async () => {
    ...
    expect(byLines[0]!.id).toMatch(/^jr_para_[0-9a-f]{12}$/);
    expect(byLines[1]!.id).toMatch(/^jr_para_[0-9a-f]{12}$/);
