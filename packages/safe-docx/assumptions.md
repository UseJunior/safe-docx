...
| A10 | Package builds and end-to-end suites pass in current repo state | Baseline release confidence | Full build/test + spec validation | `npm run build -w @usejunior/safe-docx`<br>`npm run test:run -w @usejunior/safe-docx`<br>`npm run test:run -w @usejunior/docx-primitives`<br>`openspec validate add-safe-docx-layout-format-controls --strict` | All commands pass |
