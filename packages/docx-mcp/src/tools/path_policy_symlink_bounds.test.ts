...
describe.sequential('path policy: symlink and root bounds', () => {
  ...
  test('read/write paths are constrained to allowed roots and symlink escapes are blocked', async () => {
    ...
    const allowedRoot = await createTrackedTempDir('safe-docx-path-policy-allowed-root-');
    const outsideRoot = await createTrackedTempDir('safe-docx-path-policy-outside-root-');
