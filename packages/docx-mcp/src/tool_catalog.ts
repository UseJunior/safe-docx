...
export const SAFE_DOCX_TOOL_CATALOG = [
  ...
  },
  {
    name: 'accept_changes',
    surface: 'internal',
    description: 'Accept every supported tracked change in the document body, including inserted and deleted table-row revisions. Returns acceptance stats; unresolvedRowRevisions remains 0 for supported row markers.',
    input: z.object({
      ...FILE_FIELD,
    }),
    annotations: { readOnlyHint: false, destructiveHint: true },
  ...
  },
  {
    ...
    surface: 'revisionable',
    description:
      'Compare two documents and produce a tracked-changes output document. Provide original_file_path + revised_file_path for standalone comparison, or file_path to compare session edits against the original. DOCX and ODF (.odt) support both modes. DOCX output always uses the revised archive as its package base and publishes tagged revisions; engine, strategy, reconstruction, premerge, and refinement selectors are not exposed. DOCX stats count insertions/deletions as contiguous ranges, expose tagged-token-v1 totals as insertedAtoms/deletedAtoms with atomMetricVersion, and report formatChanges separately from modifiedParagraphs. When a DOCX input difference is preserved in the output without tracked-change markup (for example a removed section\'s header or footer, or an unsupported header/footer topology), the response includes `unrepresented_changes` (objects with scope, kind, sectionIndex, and role) plus one `warnings` string per entry, and the message carries a WARNING; both fields are absent when no such difference was detected, so a clean response means the redline represents every detected difference. ODF compares at inline granularity (a modified paragraph is marked up in place — only the changed spans are struck or inserted).',
    input: z.object({
