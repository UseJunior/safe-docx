...
import { findAllByTagName } from '../baselines/atomizer/wmlElementUtils.js';
...
function canonicalizeElement(node: WmlElement): string {
  if (node.tagName === '#document') {
    return (node.children ?? []).map(canonicalizeElement).join('');
  }

  if (node.tagName === '#unknown') {
    return '';
  }

  if (node.tagName.startsWith('?')) {
    const processingName = node.tagName.slice(1);
    const attrs = Object.entries(node.attributes)
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([key, value]) => `${key}="${escapeXml(value)}"`)
      .join(' ');
    return attrs ? `<?${processingName} ${attrs}?>` : `<?${processingName}?>`;
  }

  const attrs = Object.entries(node.attributes)
    .filter(([key]) => !VOLATILE_ATTRS.has(key))
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([key, value]) => `${key}="${escapeXml(value)}"`)
    .join(' ');
  ...
  const children = (node.children ?? []).map(canonicalizeElement).join('');
  const text = node.textContent ? escapeXml(node.textContent) : '';

  if (!children && !text) {
    return attrs ? `<${node.tagName} ${attrs}/>` : `<${node.tagName}/>`;
  }

  return `${openTag}${text}${children}${closeTag}`;
}

function canonicalizeXml(xml: string): string {
  const root = parseDocumentXml(xml);
  return canonicalizeElement(root).replace(/>\s+</g, '><').trim();
}
...
function validateNumberingIntegrity(documentXml: string, numberingXml: string | null): NumberingDiagnostics {
  ...
  for (const node of ilvlNodes) {
    const rawLevel = node.attributes['w:val'];
    if (!rawLevel) {
      continue;
    }
  ...
  for (const numNode of findAllByTagName(numberingRoot, 'w:num')) {
    const abstractNode = (numNode.children ?? []).find((child) => child.tagName === 'w:abstractNumId');
    const abstractId = abstractNode?.attributes['w:val'];
    if (abstractId) {
