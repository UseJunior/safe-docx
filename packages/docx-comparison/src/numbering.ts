 ...
 */
export function detectContinuationPattern(
  ilvl: number,
  startValue: number,
  levelNumbers: number[]
): ContinuationInfo {
 ...
 */
function getNumKey(numId: number): string {
  return `num:${numId}`;
}
 ...
 */
export function processNumberedParagraph(
  state: NumberingState,
  numId: number,
  ilvl: number,
  ...
  const continuation = detectContinuationPattern(
    ilvl,
    levelInfo.start,
    counters
  );
 ...
 */
export function expandLevelText(
  lvlText: string,
  counters: number[],
 ...
 */
export function isLegalNumberingActive(
  levels: ListLevelInfo[],
  currentLevel: number
): boolean {
 ...
 */
export function expandLevelTextWithLegal(
  lvlText: string,
  counters: number[],
  levels: ListLevelInfo[],
  currentLevel: number
 ...
 */
export function parseLevelElement(lvlElement: WmlElement): ListLevelInfo {
  const ilvl = parseInt(lvlElement.attributes['w:ilvl'] ?? '0', 10);
  ...
  for (const child of lvlElement.children ?? []) {
    switch (child.tagName) {
      case 'w:start':
        start = parseInt(child.attributes['w:val'] ?? '1', 10);
        break;
      case 'w:numFmt':
        numFmt = child.attributes['w:val'] ?? 'decimal';
        break;
      case 'w:lvlText':
        lvlText = child.attributes['w:val'] ?? '';
        break;
      case 'w:isLgl':
        isLgl = child.attributes['w:val'] !== 'false' && child.attributes['w:val'] !== '0';
        break;
    }
  }
 ...
 */
export function parseAbstractNumElement(
  abstractNumElement: WmlElement
): ListLevelInfo[] {
  const levels: ListLevelInfo[] = [];

  for (const child of abstractNumElement.children ?? []) {
