...
function createMockAtom(
  ...
  const contentElement: WmlElement = {
    tagName: 'w:t',
    textContent: text,
    attributes: {},
  };
...
describe('atomLcs Regression Tests', () => {
  ...
  describe('BUG-001: MovedSource paragraph index lookup', () => {
    it('MovedSource atoms should use originalToOutputPara for paragraph index assignment', () => {
      ...
      // Create merged list
      const merged = createMergedAtomList(originalAtoms, revisedAtoms, lcsResult);
    ...
    it('MovedSource atoms should stay with their original paragraph siblings after reject', () => {
      ...
      const atomsByParagraph = new Map<number, ComparisonUnitAtom[]>();
      ...
      // Find paragraph containing "Definition A"
      let defAParagraphIdx: number | undefined;
      for (const [idx, atoms] of atomsByParagraph) {
        if (atoms.some((a) => a.contentElement.textContent === 'Definition A')) {
    ...
    it('Deleted atoms should still work correctly after MovedSource fix', () => {
      ...
      const revisedAtoms = [createMockAtom('Keep this', 0)];
