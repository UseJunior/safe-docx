 ...
 */
export function computeAtomLcs(
 ...
 */
function atomsEqual(a: ComparisonUnitAtom, b: ComparisonUnitAtom): boolean {
 ...
 */
export function markCorrelationStatus(
  original: ComparisonUnitAtom[],
  revised: ComparisonUnitAtom[],
  lcsResult: LcsResult
): void {
 ...
 */
export function createMergedAtomList(
  original: ComparisonUnitAtom[],
  revised: ComparisonUnitAtom[],
  lcsResult: LcsResult
): ComparisonUnitAtom[] {
  const merged: ComparisonUnitAtom[] = [];

  // Build maps for quick lookup
  const originalToRevised = new Map<number, number>();
  const revisedToOriginal = new Map<number, number>();
  for (const match of lcsResult.matches) {
...
export function assignUnifiedParagraphIndices(
  ...
  const allOriginalParas = [...new Set(original.map(a => a.paragraphIndex).filter(p => p !== undefined))] as number[];
  allOriginalParas.sort((a, b) => a - b);

  // Collect all unique revised paragraph indices
  const allRevisedParas = [...new Set(revised.map(a => a.paragraphIndex).filter(p => p !== undefined))] as number[];
  ...
  const originalToOutputPara = new Map<number, number>();
  const revisedToOutputPara = new Map<number, number>();
  ...
  if (emptyParagraphsWithoutMapping > 0) {
    console.log(`[DEBUG] Empty paragraphs without mapping: ${emptyParagraphsWithoutMapping}`);
  }
}
 ...
 */
export function computeLcsStats(
  original: ComparisonUnitAtom[],
  revised: ComparisonUnitAtom[],
  lcsResult: LcsResult
): LcsStats {
