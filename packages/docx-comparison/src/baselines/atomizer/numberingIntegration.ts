...
import type { ComparisonUnitAtom, WmlElement, ListLevelInfo } from '../../core-types.js';
import {
...
export function parseNumberingXml(
  ...
  try {
    ...
    if (!numbering?.children) {
      return definitions;
    }

    // Parse abstract numbering definitions
    for (const child of numbering.children) {
...
function parseAbstractNumLevels(abstractNum: WmlElement): ListLevelInfo[] {
  ...
  if (!abstractNum.children) {
    return levels;
  }

  for (const child of abstractNum.children) {
...
function findAbstractNumIdRef(numElement: WmlElement): string | null {
  if (!numElement.children) {
    return null;
  }

  for (const child of numElement.children) {
    if (child.tagName === 'w:abstractNumId') {
      return child.attributes['w:val'] || null;
    }
  }
...
function getNumIdFromAtom(atom: ComparisonUnitAtom): string | null {
  ...
  if (!pAncestor?.children) {
    return null;
  }

  // Find w:pPr
  const pPr = pAncestor.children.find((c) => c.tagName === 'w:pPr');
  if (!pPr?.children) {
    return null;
  }

  // Find w:numPr
  const numPr = pPr.children.find((c) => c.tagName === 'w:numPr');
  if (!numPr?.children) {
    return null;
  }

  // Find w:numId
  const numId = numPr.children.find((c) => c.tagName === 'w:numId');
  return numId?.attributes['w:val'] || null;
}
...
function getIlvlFromAtom(atom: ComparisonUnitAtom): number {
  ...
  // Find w:ilvl
  const ilvl = numPr.children.find((c) => c.tagName === 'w:ilvl');
  const val = ilvl?.attributes['w:val'];
