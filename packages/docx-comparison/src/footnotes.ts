 ...
 */
export function findReferencesInOrder(
  element: WmlElement,
  tagName: 'w:footnoteReference' | 'w:endnoteReference'
): WmlElement[] {
  const refs: WmlElement[] = [];
...
export function isReservedId(xmlId: string): boolean {
  return (
    xmlId === RESERVED_FOOTNOTE_IDS.SEPARATOR ||
...
export class FootnoteNumberingTracker {
  private footnoteIdToDisplayNumber: Map<string, number> = new Map();
  private endnoteIdToDisplayNumber: Map<string, number> = new Map();
  private footnoteCustomMarks: Set<string> = new Set();
  private endnoteCustomMarks: Set<string> = new Set();
   ...
   */
  private scanDocument(documentRoot: WmlElement): void {
    // Scan for footnote references
    const footnoteRefs = findReferencesInOrder(
      documentRoot,
      'w:footnoteReference'
    );
    this.buildNumbering(
      footnoteRefs,
      this.footnoteIdToDisplayNumber,
      this.footnoteCustomMarks
    );

    // Scan for endnote references
    const endnoteRefs = findReferencesInOrder(
      documentRoot,
      'w:endnoteReference'
    );
    this.buildNumbering(
      endnoteRefs,
      this.endnoteIdToDisplayNumber,
      this.endnoteCustomMarks
    );
  }
  ...
  private buildNumbering(
    ...
    for (const ref of refs) {
      ...
      // Skip reserved IDs
      if (isReservedId(xmlId)) continue;

      // Check for custom mark
      const hasCustomMark =
        ref.attributes['w:customMarkFollows'] === '1' ||
        ref.attributes['w:customMarkFollows'] === 'true';
  ...
  getDisplayNumber(xmlId: string): number | undefined {
    return (
      this.footnoteIdToDisplayNumber.get(xmlId) ??
...
export function findFootnoteById(
  ...
  for (const child of footnotesRoot.children ?? []) {
    if (child.tagName === 'w:footnote' && child.attributes['w:id'] === xmlId) {
      return child;
    }
  }
 ...
 */
export function findEndnoteById(
  endnotesRoot: WmlElement,
  xmlId: string
): WmlElement | undefined {
  ...
  for (const child of endnotesRoot.children ?? []) {
    if (child.tagName === 'w:endnote' && child.attributes['w:id'] === xmlId) {
      return child;
    }
  }
 ...
 */
export function extractNoteText(noteElement: WmlElement): string {
  const texts: string[] = [];

  function traverse(node: WmlElement): void {
    if (node.tagName === 'w:t' && node.textContent) {
