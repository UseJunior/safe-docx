...
export interface MoveMarkup {
  /** Range start element */
  rangeStart: WmlElement;
  /** Move content wrapper element */
  moveWrapper: WmlElement;
  /** Range end element */
  rangeEnd: WmlElement;
 ...
 */
export function generateMoveSourceMarkup(
  moveName: string,
  content: WmlElement[],
  ...
  const rangeStart: WmlElement = {
    tagName: 'w:moveFromRangeStart',
    attributes: {
      'w:id': rangeId,
      'w:name': moveName,
  ...
  const rangeEnd: WmlElement = {
    tagName: 'w:moveFromRangeEnd',
    attributes: {
      'w:id': rangeId,
    },
  };
 ...
 */
export function generateMoveDestinationMarkup(
  moveName: string,
  content: WmlElement[],
  options: MoveMarkupOptions
): MoveMarkup {
 ...
 */
export function createRevisionIdState(startId = 1): RevisionIdState {
  return {
    nextId: startId,
 ...
 */
export function allocateMoveIds(
  state: RevisionIdState,
  moveName: string
): { sourceRangeId: number; sourceMoveId: number; destRangeId: number; destMoveId: number } {
 ...
 */
export function fixUpRevisionIds(_state: RevisionIdState): void {
