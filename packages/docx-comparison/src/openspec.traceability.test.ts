...
function makeTextAtom(
  ...
  runProps: WmlElement[] | null = []
): ComparisonUnitAtom {
  const runChildren: WmlElement[] = [];
  if (runProps !== null) {
    runChildren.push({ tagName: 'w:rPr', attributes: {}, children: runProps });
  }
  runChildren.push({ tagName: 'w:t', attributes: {}, textContent: text });
...
function createDocumentWithFootnotes(ids: string[], customMarkIds: Set<string> = new Set()): WmlElement {
  return {
    tagName: 'w:body',
    attributes: {},
    children: ids.map((id) => ({
          ...
          children: [
            {
              ...
              attributes: (customMarkIds.has(id)
                ? { 'w:id': id, 'w:customMarkFollows': '1' }
                : { 'w:id': id }) as Record<string, string>,
            },
          ],
        },
      ],
...
describe('OpenSpec traceability: docx-comparison', () => {
  ...
  humanReadableTest.openspec('Status for format-changed content')(
    ...
    () => {
      ...
      const after = makeTextAtom('hello', CorrelationStatus.Equal, [{ tagName: 'w:b', attributes: {} }]);
  ...
  humanReadableTest.openspec('Element with text content')(
    ...
    () => {
      const element: WmlElement = { tagName: 'w:t', attributes: {}, textContent: 'Hello World' };
  ...
  humanReadableTest.openspec('Element with attributes')(
    ...
    () => {
      const element: WmlElement = {
        tagName: 'w:p',
        attributes: { 'pt14:Unid': 'abc123' },
      };
      expect(element.attributes['pt14:Unid']).toBe('abc123');
    },
  );
  ...
  humanReadableTest.openspec('Hash calculation for content identity')(
    ...
    () => {
      const atom = createComparisonUnitAtom({
        contentElement: { tagName: 'w:t', attributes: {}, textContent: 'hash me' },
        ancestors: [],
        part: PART,
      });
  ...
  humanReadableTest.openspec('Atom from inserted revision')(
    ...
    () => {
      ...
      const atom = createComparisonUnitAtom({
        contentElement: { tagName: 'w:t', attributes: {}, textContent: 'new' },
        ancestors: [ins],
        part: PART,
      });
  ...
  humanReadableTest.openspec('Atom from deleted revision')(
    ...
    () => {
      const del: WmlElement = { tagName: 'w:del', attributes: { 'w:id': '1' } };
      const atom = createComparisonUnitAtom({
        contentElement: { tagName: 'w:delText', attributes: {}, textContent: 'old' },
        ancestors: [del],
        part: PART,
      });
  ...
  humanReadableTest.openspec('Atom with ancestor tracking')(
    ...
    () => {
      ...
      const atom = createComparisonUnitAtom({
        contentElement: { tagName: 'w:t', attributes: {}, textContent: 'nested' },
  ...
  humanReadableTest.openspec('Creating atom without revision context')(
    ...
    () => {
      const atom = createComparisonUnitAtom({
        contentElement: { tagName: 'w:t', attributes: {}, textContent: 'plain' },
        ancestors: [],
        part: PART,
      });
  ...
  humanReadableTest.openspec('Move source markup structure')(
    ...
    () => {
      const content: WmlElement[] = [{ tagName: 'w:r', attributes: {}, children: [] }];
  ...
  humanReadableTest.openspec('Range IDs properly paired')(
    ...
    () => {
      ...
      expect(source.rangeStart.attributes['w:id']).toBe(source.rangeEnd.attributes['w:id']);
      expect(destination.rangeStart.attributes['w:id']).toBe(destination.rangeEnd.attributes['w:id']);
    },
  );
  ...
  humanReadableTest.openspec('Bold added')(
    ...
    () => {
      ...
      expect(getChangedPropertyNames(oldRPr, newRPr)).toContain('bold');
    },
  );
  ...
  humanReadableTest.openspec('Multiple properties changed')(
    ...
    () => {
      ...
      const changed = getChangedPropertyNames(oldRPr, newRPr);
  ...
  humanReadableTest.openspec('No format change')(
    ...
    () => {
      const before = makeTextAtom('hello', CorrelationStatus.Equal, [{ tagName: 'w:b', attributes: {} }]);
      const after = makeTextAtom('hello', CorrelationStatus.Equal, [{ tagName: 'w:b', attributes: {} }]);
      after.comparisonUnitAtomBefore = before;
  ...
  humanReadableTest.openspec('Format detection with text change')(
    ...
    () => {
      const before = makeTextAtom('hello', CorrelationStatus.Equal, []);
      const inserted = makeTextAtom('hello changed', CorrelationStatus.Inserted, [{ tagName: 'w:b', attributes: {} }]);
  ...
  humanReadableTest.openspec('Run with properties')(
    ...
    () => {
      const atom = makeTextAtom('hello', CorrelationStatus.Equal, [{ tagName: 'w:b', attributes: {} }]);
      const rPr = getRunPropertiesFromAtom(atom);
      assertDefined(rPr, 'rPr');
      expect(rPr.children?.some((child) => child.tagName === 'w:b')).toBe(true);
    },
  );
  ...
  humanReadableTest.openspec('Normalize null properties')(
    ...
    () => {
      ...
      expect(normalized.tagName).toBe('w:rPr');
  ...
  humanReadableTest.openspec('Remove existing revision tracking')(
    ...
    () => {
      const normalized = normalizeRunProperties({
        tagName: 'w:rPr',
        attributes: {},
        children: [
  ...
  humanReadableTest.openspec('Empty properties equal')(
    ...
    () => {
      const left = normalizeRunProperties(null);
      const right = normalizeRunProperties({ tagName: 'w:rPr', attributes: {}, children: [] });
  ...
  humanReadableTest.openspec('Different properties')(
    ...
    () => {
      const left = normalizeRunProperties({
        tagName: 'w:rPr',
        attributes: {},
        children: [{ tagName: 'w:b', attributes: {} }],
      });
      const right = normalizeRunProperties({
        tagName: 'w:rPr',
        attributes: {},
        children: [{ tagName: 'w:i', attributes: {} }],
      });
      expect(areRunPropertiesEqual(left, right)).toBe(false);
    },
  );
  ...
  humanReadableTest.openspec('Format change markup structure')(
    ...
    () => {
      ...
      expect(markup.attributes['w:author']).toBe('Tester');
      expect(markup.attributes['w:date']).toBeDefined();
      expect(markup.children?.[0]?.tagName).toBe('w:rPr');
    },
  );
  ...
  humanReadableTest.openspec('Bold added markup')(
    ...
    () => {
      ...
      const rPrChange = generateFormatChangeMarkup({
        ...
        newRunProperties: { tagName: 'w:rPr', attributes: {}, children: [{ tagName: 'w:b', attributes: {} }] },
      ...
      const rPr = run.children?.find((child) => child.tagName === 'w:rPr');
      assertDefined(rPr, 'rPr');
      const inserted = rPr.children?.find((child) => child.tagName === 'w:rPrChange');
      assertDefined(inserted, 'rPrChange');
      const oldRPr = inserted.children?.[0];
      assertDefined(oldRPr, 'oldRPr');
      expect(oldRPr.children ?? []).toHaveLength(0);
    },
  );
  ...
  humanReadableTest.openspec('Bold removed markup')(
    ...
    () => {
      const run: WmlElement = {
        ...
        attributes: {},
        children: [
          { tagName: 'w:rPr', attributes: {}, children: [] },
          { tagName: 'w:t', attributes: {}, textContent: 'text' },
        ],
      };
      ...
      const rPrChange = generateFormatChangeMarkup({
        ...
        newRunProperties: { tagName: 'w:rPr', attributes: {}, children: [] },
        changedProperties: ['bold'],
      }, {
        author: 'Tester',
        dateTime: new Date('2026-01-01T00:00:00.000Z'),
      ...
      const oldRPr = inserted.children?.[0];
      assertDefined(oldRPr, 'oldRPr');
      expect(oldRPr.children?.some((child) => child.tagName === 'w:b')).toBe(true);
    },
  );
  ...
  humanReadableTest.openspec('Unknown property name')(
    ...
    () => {
      ...
      const newRPr = normalizeRunProperties({
        tagName: 'w:rPr',
        attributes: {},
        children: [{ tagName: 'w:emboss', attributes: {} }],
      });
  ...
  humanReadableTest.openspec('Building footnote mapping')(
    ...
    () => {
      ...
      expect(refs.map((ref) => ref.attributes['w:id'])).toEqual(['9', '3', '5']);
    },
  );
});
