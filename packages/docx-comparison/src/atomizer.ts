...
export function hashElement(element: WmlElement): string {
  ...
  const sortedAttrs = Object.entries(element.attributes)
...
export function extractAncestorUnids(ancestors: WmlElement[]): string[] {
  ...
  for (const ancestor of ancestors) {
    const unid = ancestor.attributes['w:Unid'];
    if (unid) {
      unids.push(unid);
    }
  }
...
function cloneWmlElement(element: WmlElement): WmlElement {
  const clone: WmlElement = {
    tagName: element.tagName,
    attributes: { ...element.attributes },
  };
  if (element.textContent !== undefined) clone.textContent = element.textContent;
  if (element.children) clone.children = element.children.map((c) => cloneWmlElement(c));
  return clone;
}
...
function createEmptyParagraphAtomWithContext(
  ...
  const virtualElement: WmlElement = {
    tagName: EMPTY_PARAGRAPH_TAG,
    attributes: {},
    textContent: '',
    children: [],
  };
  ...
  const pPr = paragraphElement.children?.find((c) => c.tagName === 'w:pPr');
...
function atomizeTreeInternal(
  ...
  if (isLeafNode(node)) {
    const atom = createComparisonUnitAtom({
      contentElement: options.cloneLeafNodes ? cloneWmlElement(node) : node,
      ancestors,
      part,
    });
    atoms.push(atom);
 ...
 *
 * fast-xml-parser doesn't provide parent references, so we add them
 * during a post-processing pass.
 *
...
export function assignParagraphIndices(atoms: ComparisonUnitAtom[]): void {
  const paragraphToIndex = new Map<WmlElement, number>();
  let nextIndex = 0;
...
function isFieldBegin(atom: ComparisonUnitAtom): boolean {
  return (
    ...
    atom.contentElement.attributes['w:fldCharType'] === 'begin'
  );
}
...
function isFieldSeparate(atom: ComparisonUnitAtom): boolean {
  return (
    atom.contentElement.tagName === 'w:fldChar' &&
    atom.contentElement.attributes['w:fldCharType'] === 'separate'
  );
}
...
function isFieldEnd(atom: ComparisonUnitAtom): boolean {
  return (
    atom.contentElement.tagName === 'w:fldChar' &&
    atom.contentElement.attributes['w:fldCharType'] === 'end'
  );
}
...
function extractVisibleText(atoms: ComparisonUnitAtom[]): string {
  return atoms
    ...
    .map((a) => a.contentElement.textContent ?? '')
    .join('');
}
...
export function collapseFieldSequences(
  ...
  while (i < atoms.length) {
    ...
    if (isFieldBegin(atom)) {
      ...
      const virtualElement: WmlElement = {
        tagName: 'w:t', // Use w:t so it can merge with adjacent text
        attributes: {},
...
function splitAtomIntoWords(atom: ComparisonUnitAtom): ComparisonUnitAtom[] {
  ...
  for (const part of parts) {
    ...
    const wordElement: WmlElement = {
      tagName: 'w:t',
      attributes: { ...atom.contentElement.attributes },
      textContent: part,
    };
...
function getRunProperties(atom: ComparisonUnitAtom): WmlElement | undefined {
  ...
  return run.children?.find((c) => c.tagName === 'w:rPr');
}
...
function mergeIntoAtom(target: ComparisonUnitAtom, source: ComparisonUnitAtom): void {
  ...
  const newText =
    (target.contentElement.textContent ?? '') +
    (source.contentElement.textContent ?? '');
  target.contentElement.textContent = newText;
