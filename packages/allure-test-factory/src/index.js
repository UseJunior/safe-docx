...
export function createAllureTestHelpers(config) {
  ...
  function wrapWithAllure(fn, explicitName, defaults) {
    ...
    return (async (...args) => {
      ...
      const allureRuntime = getAllureRuntime();
      if (allureRuntime) {
        // Auto-strip "Scenario:" prefix from display name when no explicit title is set
        if (typeof allureRuntime.displayName === 'function') {
          const hasExplicitTitle = typeof effectiveDefaults?.title === 'string'
            && effectiveDefaults.title.length > 0;
          if (!hasExplicitTitle) {
            const resolvedName = resolveStoryLabel(explicitName, nameParts);
            if (/^Scenario:\s*/i.test(resolvedName)) {
              await allureRuntime.displayName(normalizeScenarioText(resolvedName));
            }
          }
        }
