  ...
  private ensureInnerReporter(): Promise<AllureReporterLike | null> {
    ...
    this.innerPromise = (async () => {
      ...
      if (!innerReporterPath) {
        ...
        console.warn('[safe-docx-mcpb] Missing innerReporterPath for Allure compatibility reporter.');
        return null;
      }
      ...
      try {
        ...
        if (typeof ReporterCtor !== 'function') {
          // eslint-disable-next-line no-console
          console.warn(
            `[safe-docx-mcpb] Allure reporter at '${innerReporterPath}' has no default class export.`,
          );
          return null;
        }
        ...
        // eslint-disable-next-line no-console
        console.warn(
          `[safe-docx-mcpb] Failed to load Allure reporter '${innerReporterPath}': ${String(error)}`,
        );
        return null;
      }
    })();
