...
| `revisionGrouping` | Internal low-level | Markdoc always requests bounded U+0020 bridge coalescing in eligible replacement gaps; other comparator callers retain the token-minimal default. Covered by `src/tagged/taggedTreeSerializer.test.ts` and `../docx-markdoc/src/readable-redline-grouping.test.ts`. |
