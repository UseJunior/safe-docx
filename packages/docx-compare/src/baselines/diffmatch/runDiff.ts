...
import type { RunInfo, RunDiffResult, DiffOp } from '@usejunior/docx-core';
 ...
 */
export function computeWordDiff(original: string, revised: string): DiffOp[] {
  ...
  return charsToTokens(diffs, lineArray) as DiffOp[];
}
...
function tokensToChars(
  ...
  const tokenSet: Map<string, number> = new Map();
...
/**
 * Represents a position in the run array.
 */
 ...
 */
function offsetToRunPosition(runs: RunInfo[], offset: number): RunPosition {
  let charCount = 0;
 ...
 */
export function splitRun(run: RunInfo, offset: number): [RunInfo, RunInfo] {
  const before: RunInfo = {
 ...
 */
export function extractText(runs: RunInfo[]): string {
 ...
 */
export function countChanges(diffs: DiffOp[]): { insertions: number; deletions: number } {
  let insertions = 0;
  let deletions = 0;
