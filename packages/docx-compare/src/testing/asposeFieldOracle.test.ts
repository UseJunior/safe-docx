...
interface Snapshot {
  ...
  fieldCases: Array<{ id: string; originalSha256: string; revisedSha256: string; classification: string; deletedFldChars: number; insertedFldChars: number; outsideRevisionFldChars: number }>;
  ilpa: { originalSha256: string; revisedSha256: string; measured: Record<string, any> };
}
...
describe('Aspose field differential oracle trust boundary', () => {
  ...
  test.openspec('A cached-result-only change preserves field scaffolding')('classifies the NUMPAGES cache update as result-only', () => {
    ...
    expect(verdict).toMatchObject({ classification: 'cached-result-only', deletedFldChars: 0, insertedFldChars: 0, outsideRevisionFldChars: 3 });
  });
  ...
  test.openspec('CI validates evidence without Aspose or its license')('validates committed provenance using only repository files', () => {
    ...
    expect(snapshot.ilpa.originalSha256).toBe(hash(resolve(repoRoot, 'tests/test_documents/redline/ILPA-Model-Limited-Partnership-Agreement-WOF_v2.docx')));
    expect(snapshot.ilpa.revisedSha256).toBe(hash(resolve(repoRoot, 'tests/test_documents/redline/ILPA-Model-Limited-Parnership-Agreement-Deal-By-Deal_v1.docx')));
  });
  ...
  test.openspec('The ILPA trust boundary records agreement and divergence')('pins the measured agreements and the non-authoritative divergence', () => {
    const measured = snapshot.ilpa.measured;
