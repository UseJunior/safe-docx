...
import { extractListLabel, stripListLabel, type ListLabelResult, LabelType } from './list_labels.js';
import { parseNumberingXml, type NumberingModel, type NumberingCounters, computeListLabelForParagraph } from './numbering.js';
...
export function renderToon(nodes: DocumentViewNode[]): string {
  ...
  for (let idx = 0; idx < paragraphs.length; idx++) {
    ...
    try {
      ...
      for (const tr of trs) {
        ...
        const isHeaderStyle = fmt.bold || fmt.underline;
        if (isHeaderStyle && (headerText || '').length > 0) continue;
        bodyFmt = fmt;
        break;
      }
    } catch {
