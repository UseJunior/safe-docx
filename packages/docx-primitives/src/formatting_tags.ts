...
function desiredTagsForRun(run: AnnotatedRun, baseline: FormattingBaseline): ActiveTags {
  if (run.isHeaderRun) {
...
export function emitFormattingTags(params: {
  ...
  const skipChars = new Set<number>(); // chars to omit (quote chars)
  const insertBefore = new Map<number, string>(); // tags to insert before a char
  const insertAfter = new Map<number, string>(); // tags to insert after a char
...
function emitFormattingTagsSimple(runs: AnnotatedRun[], baseline: FormattingBaseline): string {
  const out: string[] = [];
