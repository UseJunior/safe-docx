...
function isBarrierElement(el: Element): boolean {
  if (el.namespaceURI !== OOXML.W_NS) return false;
  return BARRIER_LOCALS.has(el.localName) || TC_WRAPPER_LOCALS.has(el.localName);
}
