...
export default class AllureVitestCompatReporter implements Reporter {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (packageName) {
          ...
          //    Before: ['DOCX Primitives', 'merge_runs.test.ts', 'merge_runs', 'safety barriers']
          //    After:  ['DOCX Primitives', 'merge_runs', 'safety barriers']
  ...
  private ensureInnerReporter(): Promise<AllureReporterLike | null> {
    ...
    this.innerPromise = (async () => {
      ...
      if (!innerReporterPath) {
        ...
        console.warn('[docx-primitives-ts] Missing innerReporterPath for Allure compatibility reporter.');
        return null;
      }
      ...
      try {
        ...
        if (typeof ReporterCtor !== 'function') {
          // eslint-disable-next-line no-console
          console.warn(
            `[docx-primitives-ts] Allure reporter at '${innerReporterPath}' has no default class export.`,
          );
          return null;
        }
        ...
        // eslint-disable-next-line no-console
        console.warn(
          `[docx-primitives-ts] Failed to load Allure reporter '${innerReporterPath}': ${String(error)}`,
        );
        return null;
      }
    })();
  ...
  private async ensureResultsDirClean(): Promise<void> {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn(
        `[docx-primitives-ts] Failed to clean results dir '${resultsDir}': ${String(error)}`,
      );
    }
  }
}
