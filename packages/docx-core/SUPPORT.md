...
| `comments.ts` — `addComment` | `packages/docx-core/src/primitives/comments.ts` | `w:ins` | The comment anchor inserted into the body story is revisionable content. Root comment text in `comments.xml` is currently plain comment content; tracking that side-part text is deferred from #121. Companion package bootstrap is listed in Table B. **Verified by [120.8] (#143) regression test.** |
...
| `add_comment` | `packages/docx-mcp/src/tools/add_comment.ts` | `w:ins` (root-comment mode only) | Root comment anchors are part of the revisionable surface. Root comment text in `comments.xml` is currently plain comment content; tracking that side-part text is deferred from #121. **Reply-mode** dispatches through `comments.ts` `addCommentReply` which is Table B (side-part metadata only; no body revision marker). Missing comment infrastructure is also a Table B companion. **Verified by [120.8] (#143) regression test.** |
...
- `validate_revisions.ts` — validator over AI-emitted and pre-existing OOXML revision markup; MCP write and save gates use it to hard-error on session-owned malformed revisions while leaving pre-existing defects as warnings.
