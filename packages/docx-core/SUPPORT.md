...
| `sections.ts` — `setSectionPageNumberStart`, `updateSectionProperties` | `packages/docx-core/src/primitives/sections.ts` | `w:sectPrChange` | DOCX section page-number and page-setup mutations preserve unrelated section properties and atomically capture the prior `w:sectPr`. **Verified by the #654 canonical-emission regression tests.** |
