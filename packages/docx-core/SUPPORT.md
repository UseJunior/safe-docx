...
| `table_rows.ts` — `insertTableRow`, `deleteTableRow` | `packages/docx-core/src/primitives/table_rows.ts` | `w:ins`, `w:del` row markers plus paragraph-mark and run-content revisions in comparison-derived output | Bounded whole-row topology changes for simple rectangular body tables. The primitives enforce grid, merge, nesting, revision, and final-row constraints; canonical Markdoc exposes them, while MCP row editing remains unsupported. **Verified by the #1001 structural primitive regressions and #998 Markdoc/comparison projection regressions.** |
...
| `sections.ts` — `setSectionPageNumberStart`, `updateSectionProperties`, `insertSectionBreak` | `packages/docx-core/src/primitives/sections.ts` | `w:ins` paragraph mark, `w:sectPrChange` | DOCX section page-number/page-setup mutations and anchored section splitting preserve unrelated section properties and header/footer bindings. Inserted topology is reviewable as a dedicated boundary paragraph; following-section overrides atomically capture the prior `w:sectPr`. **Verified by the #654 canonical-emission and accept/reject tests.** |
...
| `format_section` | `packages/docx-mcp/src/tools/format_section.ts` | `w:sectPrChange` | DOCX-only page-number, page-size/orientation, and margin updates delegate to one atomic section primitive and retain the prior `w:sectPr` in tracked output. **Verified by the #654 canonical-emission MCP regression tests.** |
| `insert_section_break` | `packages/docx-mcp/src/tools/insert_section_break.ts` | `w:ins` paragraph mark, optional `w:sectPrChange` | DOCX-only anchored section splitting preserves page setup and relationship references, projects shifted section indexes, and restores original topology on reject. **Verified by the #654 MCP and core accept/reject tests.** |
...
- `insert_section_break.ts` — Table A
