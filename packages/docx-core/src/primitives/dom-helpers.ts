 ...
 */
export function getLeafText(el: Element): string | undefined {
  for (let i = 0; i < el.childNodes.length; i++) {
    const child = el.childNodes[i]!;
    if (child.nodeType === NODE_TYPE.TEXT) return child.nodeValue ?? '';
  }
  return undefined;
}
...
export function childElements(el: Element | Node): Element[] {
  ...
  for (let i = 0; i < el.childNodes.length; i++) {
    const child = el.childNodes[i]!;
    if (child.nodeType === NODE_TYPE.ELEMENT) result.push(child as Element);
  }
  return result;
}
 ...
 */
export function insertAfterElement(refEl: Node, newEl: Node): void {
 ...
 */
export function wrapElement(target: Node, wrapper: Element): void {
 ...
 */
export function removeAllByTagName(root: Element, tagName: string): number {
 ...
 */
export function unwrapAllByTagName(root: Element, tagName: string): number {
 ...
 */
export function findAllByTagName(root: Element, tagName: string): Element[] {
 ...
 */
export function renameElement(el: Element, newTagName: string): Element {
 ...
 */
export function createWmlElement(
  doc: Document,
  localTag: string,
