...
export function hasDefinitionTags(text: string): boolean {
...
export function emitDefinitionTagsFromString(text: string): string {
  if (!text) return text;
  ...
  for (let i = matches.length - 1; i >= 0; i--) {
    ...
    if (closeQuoteIdx >= tagged.length) continue;
...
export function findInlineDefinitionSpan(text: string): { term: string; term_start: number; term_end: number } | null {
...
export function stripDefinitionTags(text: string): string {
 ...
 */
export function detectDefinitionSpans(text: string): Array<{ start: number; end: number; term: string }> {
  if (!text) return [];
