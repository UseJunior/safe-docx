...
function findAncestorByTag(atom: ComparisonUnitAtom, tagName: string): Element | undefined {
 ...
 */
function collectAdjacentSourceBookmarkMarkers(sourceRun: Element): Element[] {
...
function findTreeRoot(node: Element): Element {
...
function findAncestor(node: Element | undefined, tagName: string): Element | undefined {
...
function hasAncestorTag(node: Element | undefined, tagNames: ReadonlySet<string>): boolean {
...
function paragraphHasParaInsMarker(paragraph: Element | undefined): boolean {
  if (!paragraph || paragraph.tagName !== 'w:p') {
    return false;
  }
  const pPr = findChildByTagName(paragraph, 'w:pPr');
...
function markerSurvivesReject(marker: Element, context?: BookmarkSurvivalContext): boolean {
 ...
 */
function getAtomRuns(atom: ComparisonUnitAtom): Element[] {
  if (!atom.collapsedFieldAtoms || atom.collapsedFieldAtoms.length === 0) {
 ...
 */
function getRunInsertionAnchor(run: Element): Element {
 ...
 * distinct revisions.
 */
function mergeAdjacentTrackChangeSiblings(root: Element): void {
