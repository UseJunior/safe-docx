...
export const PREFIX_MAP: Record<string, string> = Object.fromEntries(
  Object.entries(NSMAP).map(([k, v]) => [v, k])
);
 ...
 */
export function clarkToPrefixed(clarkName: string): string {
 ...
 */
export function prefixedToClark(prefixedName: string): string {
  if (prefixedName.includes(':')) {
    const [prefix, local] = prefixedName.split(':', 2) as [string, string];
 ...
 * localName(W_P) // => 'p'
 */
export function localName(clarkName: string): string {
  if (clarkName.startsWith('{')) {
    const nsEnd = clarkName.indexOf('}');
    if (nsEnd !== -1) {
