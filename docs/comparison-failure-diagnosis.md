...
- **D1 resolves what the resolver resolves.** `extractEffectiveRunFormatting` layers direct `w:rPr`, the `w:rStyle` chain, the paragraph mark's `w:rPr`, and the paragraph style's `basedOn` chain, each property independently. It does not reach `w:docDefaults`, table-style run properties, numbering-level `rPr`, or theme font references — swapping `w:asciiTheme` for the explicit font the theme maps to reads as a font change, because equivalence would need `theme1.xml`. Toggle properties (bold, italic) resolve as a nearest-declaration cascade — an explicit off at a nearer tier beats a more distant on — not as full OOXML toggle evaluation, where style-level toggles combine by XOR; repeated toggle declarations across style levels can resolve differently here than in Word. Underline is reduced to on/off, so an underline style-to-style change (single to dotted) is no longer a finding — the declared-properties projection this replaced ([#684](https://github.com/UseJunior/safe-docx/issues/684)) caught that corner but missed every style-carried loss. Color hex compares case-insensitively.
