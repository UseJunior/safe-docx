...
use `@usejunior/docx-release-verifier` for deterministic package, exact replay,
expectation, native-comment, mutation-control, and renderer-evidence gates.
No separate theorem prover or compiled checker is required.

The release verifier validates the artifacts presented to it. It is not a
proof of the TypeScript source code, visual fidelity, or the complete
ECMA-376 standard.
