...
- [ ] 1.3 Add `@conformance` documentation, Allure conformance metadata, registry coverage, and native accept/reject tests for `w:rPrChange`.
...
- [ ] 3.1 Map every declaration wholly to one common source/revised alignment interval.
...
- [ ] 4.1 Prove comparison emits native `w:rPrChange` with the prior direct properties and no text revisions attributable solely to a format-only span.
- [ ] 4.2 Verify source/reject and clean/accept text plus semantic formatting projections.
- [ ] 4.3 Add bounded certificate diagnostics for declared spans, changed properties, emitted property ranges, and format-only text-revision violations.
