...
- [x] 1.1 Add byte-native typed marker kinds, physical-story/event ordinals,
...
- [x] 1.2 Define the exact semantic source domain from retained main,
...
- [x] 1.3 Define one structural-recursive bounded scan for starts, ends, and
  references with explicit event/source/occurrence counters and first-crossing
  evidence.
- [x] 1.4 Define the per-canonical-ID point/ranged disjunction, exact-one
...
- [x] 1.5 Add semantic negative witnesses for duplicate references/endpoints,
  ...
  endpoint-only IDs.
- [x] 1.6 Add a positive semantic witness proving that one unique direct
...
- [x] 2.1 Implement and prove the seven exact protocol-v7 semantic theorem
...
- [x] 2.5 Extend exact-signature, module-provenance, recursive dependency,
  missing-required, forbidden-extra, no-LeanSpike, foundational-axiom, and
  zero-sorry audits. Build imported project modules from current source before
  reading theorem closures, reject stale `.olean` certification with an
  executable regression, and include the concrete aggregate/inhabitance
  witnesses in the exact empty-axiom target set.
- [x] 2.6 Prove typed/executable agreement for a selected Comments realization
...
- [x] 3.1 Replace the comment source `zipIdx`/copied-list execution path with
...
- [x] 3.2 Collect starts, ends, and references together; enforce the exact
  ...
  work.
- [x] 3.3 Reuse the selected Comments realization and direct-definition scan
  without a package read, snapshot write, extraction, parse, relationship walk,
  story discovery, or second source scan.
- [x] 3.4 Implement deterministic canonical topology issues, code-specific
...
- [x] 4.1 Extend public comment inventory/failure types additively and keep
...
- [x] 4.2 Implement strict protocol-v7 decoding for exact keys, canonical JSON,
...
- [x] 4.3 Reject v6, unknown/extra/missing fields, noncanonical wire IDs,
...
- [x] 4.4 Project optional human-readable paired-or-point profile evidence and
...
- [x] 4.5 Assert one package read/snapshot write per side and one retained
...
- [x] 4.6 Add strict decoder and public-projection regressions for a unique
...
- [x] 5.1 Add positive point, same-paragraph, cross-paragraph, crossing-range,
  and canonical-alias fixtures.
- [x] 5.2 Add independent range fixtures in main, every selected header/footer
  physical story/role, footnotes, and endnotes, asserting physical-story
  identity and deterministic event order.
- [x] 5.3 Add missing, malformed, and overlong ID tests for starts, ends, and
  references.
- [x] 5.4 Add duplicate reference/start/end, orphan start/end, reversed,
...
- [x] 5.5 Add maximum and over-limit start/end/reference/unique-ID tests,
...
- [x] 5.6 Add strict decoder mutation coverage for every protocol field,
  inventory count/equation, issue code/extras/order/coalescing/sentinel, and
  ordinary/terminal boundary.
- [x] 5.7 Extend Lean/TypeScript differential tests so independent typed,
...
- [x] 6.1 Extend a real source-derived inplace DOCX triple with point,
...
- [x] 6.2 Add compared-only real-DOCX mutations for malformed/overlong/alias,
...
- [x] 6.3 Add a real-DOCX selected Comments part with one unique unreferenced
...
- [x] 6.4 Run the complete checked-in NVCA-derived triple through the exact
...
- [x] 6.5 Add maximum-marker and large irrelevant-event stack witnesses under a
  ...
  Include a missing-Comments-relationship fixture with the first marker near
  the beginning and large later events/stories, plus structural processed-count
  evidence that the later work was not invoked.
- [x] 6.6 Enforce a 120-second limit and checker peak RSS below 1.5 GiB for the
...
- [x] 7.1 Add exact vendored-schema/registry traces for
...
- [x] 7.2 Add `@conformance` and structured test labels only for ECMA-backed ID
  typing and element semantics in §§17.13.4.3, 17.13.4.4, 17.13.4.5, and
  17.18.10.
- [x] 7.3 Record orphan-endpoint rejection as the stronger Safe-DOCX
...
- [x] 7.4 Regenerate conformance documentation and capability/coverage
  projections and pass drift checks.

## 8. CI, review, and smoke

- [x] 8.1 Run strict OpenSpec/spec coverage, conformance citation/document,
...
- [x] 8.2 Audit CI commands and artifacts for the fixed 8 MiB stack,
