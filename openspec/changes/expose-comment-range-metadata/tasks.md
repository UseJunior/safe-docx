...
- [ ] 1.1 Extend `McpComment` in `packages/docx-mcp/src/tools/get_comments.ts` with optional
...
- [ ] 1.2 Update the `get_comments` entry in `packages/docx-mcp/src/tool_catalog.ts` to
      document the optional range fields, and regenerate
      `packages/docx-mcp/docs/tool-reference.generated.md`
- [ ] 1.3 Add traceability tests (`TEST_FEATURE = 'expose-comment-range-metadata'`) covering
...
- [ ] 1.4 Run build, lint, tests, spec-coverage, and tool-docs checks
