...
| `advanced-revision.moves-ranges` | `conformance-gap` | emit=`implemented`<br>accept=`conformance-gap`<br>reject=`conformance-gap`<br>validate=`implemented`<br>comparison.inplace=`implemented`<br>comparison.rebuild=`implemented`<br>reconstruction.inplace=`preservation-only`<br>reconstruction.rebuild=`preservation-only`<br>lean.textFieldProjection=`non-goal`<br>lean.advancedRecordSemantics=`implemented` |
