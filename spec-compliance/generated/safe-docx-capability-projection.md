...
Exact covered surfaces: word/document.xml text, field-marker, and tracked-move-range token stream (required); word/footnotes.xml user-note text, field-marker, and tracked-move-range token stream (optional; absent sides modeled as empty when any side is present); word/endnotes.xml user-note text, field-marker, and tracked-move-range token stream (optional; absent sides modeled as empty when any side is present).
